  ...
  return (
    <header className="border-b border-slate-800 bg-slate-950/90 backdrop-blur-md sticky top-0 z-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div ref={searchRef} className="relative flex-1 max-w-lg mx-auto md:mx-4">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-slate-900/90 border border-slate-700/80 rounded-xl pl-10 pr-28 sm:pr-32 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all font-mono shadow-inner"
            />
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
              {isSearching && (
                <Loader2 className="h-3.5 w-3.5 text-cyan-400 animate-spin mr-0.5" />
              )}
              
              <button
                type="button"
                onClick={onOpenGuide}
                className="flex items-center space-x-1 px-2 py-1 rounded-lg bg-slate-800/90 hover:bg-cyan-950/80 text-slate-300 hover:text-cyan-300 border border-slate-700/80 hover:border-cyan-600/70 text-[11px] font-medium transition-all group cursor-pointer shadow-sm"
              ...
              <kbd className="hidden md:inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-mono text-slate-400 bg-slate-800 border border-slate-700 rounded shadow-sm pointer-events-none">
