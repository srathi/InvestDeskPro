    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">Curated Investment Bundles</h2>
          <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-950 text-amber-300 border border-amber-800 rounded-full uppercase">
            Thematic Baskets
          </span>
        </div>
        ...
        {bundles.map((bundle) => (
          ...
          >
            <div className="space-y-3">
              ...
              {/* Risk Level Badge */}
              <div>
                <span className="px-2.5 py-0.5 rounded-md text-[10px] font-semibold bg-slate-900 border border-slate-800 text-slate-300">
              ...
              <div className="grid grid-cols-3 gap-2 p-3 rounded-xl bg-slate-950/70 border border-slate-800/80 font-mono text-center">
                <div>
                  ...
                  <span className="text-xs font-bold text-cyan-400">{bundle.avg_pe}x</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-500 uppercase block font-semibold">Avg ROE</span>
                  <span className="text-xs font-bold text-emerald-400">{bundle.avg_roe}%</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-500 uppercase block font-semibold">1Y Return</span>
                  <span className="text-xs font-bold text-amber-400">+{bundle.avg_1y_return}%</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {bundle.tickers.map((t) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-slate-950 border border-slate-800 hover:border-cyan-700 hover:text-cyan-300 text-slate-300 text-xs font-mono font-medium transition-colors flex items-center gap-1"
                  >
                    <span>{t}</span>
